<?php namespace App\Http\Controllers;
	...
	class AdminUserListController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Level","name"=>"level","join"=>"levels,nama"];
			$this->col[] = ["label"=>"Referral Name","name"=>"ref_id","join"=>"users,name"];
			...
			$this->form[] = ['label'=>'Referral','name'=>'ref_id','type'=>'select2','validation'=>'integer|min:0','width'=>'col-sm-10','datatable'=>'users,name','datatable_format'=>"name,' - ',username"];
			...
			//$this->form[] = ['label'=>'Level','name'=>'level','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10'];
			//$this->form[] = ['label'=>'Referral','name'=>'ref_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','help'=>'Minimum 5 characters. Please leave empty if you did not change the password.'];
			# OLD END FORM
	    ...
	    */
        ...
        {
            $uuid = Str::random(20);
            $check = DB::table('users')->where('uuid', $uuid)->first();
            if($check){
                $this->generateUUID();
            }
            return $uuid;
