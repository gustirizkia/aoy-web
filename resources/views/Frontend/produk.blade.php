            ...
            <div class="col-span-12 md:col-span-3">
                ...
                <div class="border pl-4 py-4 shadow rounded-lg md:block hidden whitespace-nowrap">
                    ...
                    <div class="mt-6">
                        @if (!Request::has('q'))
                            <div
                                class=" cursor-pointer bg-gradient-to-r from-white to-purple-300 py-2 font-semibold text-neutral-800 relative">
                                Semua Produk
                                <div class="bg-primary h-full absolute right-0 w-2 top-0"></div>
                            </div>
                        @else
                            <a href="/produk{{ Request::has('search') ? '?search=' . Request::get('search') : '' }}"
                                class="text-gray-500 cursor-pointer  block">Semua Produk</a>
                        @endif
                    </div>
                    @foreach ($kategori as $item)
                        <div class="mt-6 whitespace-nowrap">
                            ...
                            @if (Request::has('q'))
                                ...
                                @else
                                    <a href="?q={{ $item->slug }}{{ Request::has('search') ? '&search=' . Request::get('search') : '' }}"
                                        class="text-gray-500 cursor-pointer  block">{{ $item->nama }}</a>
                                @endif
                            @else
                                <a href="?q={{ $item->slug }}{{ Request::has('search') ? '&search=' . Request::get('search') : '' }}"
                                    class="text-gray-500 cursor-pointer  block">{{ $item->nama }}</a>
