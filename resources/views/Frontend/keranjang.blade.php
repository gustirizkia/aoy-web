...
@section('content')
    <div class="" x-data="funData">
        <form action="{{ route('createInv') }}" method="POST">
            @csrf
                                ...
                                <div class="justify-between flex border p-2 md:p-4 rounded-lg w-full relative mb-6">
                                    ...
                                    <div class="z-20  items-center justify-between">
                                        ...
                                        <div class=" absolute bottom-0 right-0 z-10 p-4 hidden md:block">
                                            <div
                                                class="flex relative justify-between items-center p-1 border rounded-full w-full">
                                                <img src="{{ asset('gambar/icon/min-circle.png') }}" alt=""
                                                    class="opacity-60 w-4 z-10 mr-6 cursor-pointer"
                                                    @click="handleAddCart(produk.produk_id, 'minus')">
                                                ...
                                                <div class="">
                                                    <img src="{{ asset('gambar/icon/plus-circle.png') }}" alt=""
                                                        class="w-4 z-10 ml-6 relative cursor-pointer"
                                                        @click="handleAddCart(produk.produk_id)">
                                                </div>
                                            </div>
                                    ...
                                    <div class="absolute bottom-0 right-0 p-2 md:hidden">
                                        <div
                                            class="flex relative justify-between items-center p-1 border rounded-full w-full">
                                            <img src="{{ asset('gambar/icon/min-circle.png') }}" alt=""
                                                class="opacity-60 w-4 z-10 mr-3 cursor-pointer"
                                                @click="handleAddCart(produk.produk_id, 'minus')">
                                            ...
                                            <div class="">
                                                <img src="{{ asset('gambar/icon/plus-circle.png') }}" alt=""
                                                    class="w-4 z-10 ml-3 relative cursor-pointer">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-span-4 md:block hidden">
                                <div class="border rounded-xl p-4 md:sticky md:top-20 ">
                                    ...
                                    <div class="font-bold text-gray-800 flex justify-between">
                                        ...
                                        <div x-text="'Rp'+numberWithCommas(potongan.total_potongan)" class=""></div>
                                    </div>
                                    <div class="">
    ...
    <script>
        function funData() {
            return {
                ...
                potongan: {
                    harga: {{ $level ? $level->potongan_harga : 0 }},
                    tipe_potongan: "{{ $level ? $level->tipe_potongan : 0 }}",
                    total_potongan: 0,
                    diskon: 0,
                },
                ...
                init() {
                    @if(!$alamat)
                        window.location.replace("/dashboard/profile?keranjang=true");
                    @endif
                    ...
                    this.produks.forEach(item => {
                        ...
                        if (this.potongan.tipe_potongan === 'fix') {
                            ...
                            this.potongan.total_potongan = this.totalHarga - this.potongan
                                .diskon;
                        } else if (this.potongan.tipe_potongan === 'persen') {
                            ...
                            this.potongan.total_potongan = this.totalHarga;
                        }
                    });

                    this.$watch("produks", (value) => {
                        this.potongan =  {
                            harga: {{ $level ? $level->potongan_harga : 0 }},
                            tipe_potongan: "{{ $level ? $level->tipe_potongan : 0 }}",
                            total_potongan: 0,
                        ...
                        this.produks.forEach(item => {
                            ...
                            if (this.potongan.tipe_potongan === 'fix') {
                                ...
                                console.log('this.totalHarga', this.totalHarga)
                                this.potongan.total_potongan = this.totalHarga;
                            }
                        });
                    })
